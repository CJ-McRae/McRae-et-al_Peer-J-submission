...
OT_plot <- ggplot(data=OT_TEMP, 
  ...
  labs(title="A. Outlet reef (warmed and thermally variable site)", y="Temperature (°C)", x="Month")
...
WT_plot <- ggplot(data=WT_TEMP, 
  ...
  scale_x_date(date_labels="%b",date_breaks  ="1 month")+
  scale_y_continuous(limits=c(22, 34), breaks=c(22, 24, 26, 28, 30, 32, 34))+
  labs(title="B. Wanlitung reef (thermally stable site)", y="Temperature (°C)", x="Month")
...
daymean_plot <- 
  ...
  scale_x_date(date_labels="%b",date_breaks  ="1 month")+
  labs(title="C. Mean daily temperature", y="Temperature (°C)", x="")+
  theme(text=element_text(size=14,  family="serif"))
...
daymean_range_plot <- 
  ...
  theme(axis.text.x = element_text(angle=0, margin = margin(t=5, r=50)))+
  scale_x_date(date_labels="%b",date_breaks  ="1 month")+
  labs(title="D. Daily temperature range", y="Temperature (°C)", x="")+
  theme(text=element_text(size=14,  family="serif"))
...
daymean_max_plot <- 
  ...
  theme(axis.text.x = element_text(angle=0, margin = margin(t=5, r=50)))+
  scale_x_date(date_labels="%b",date_breaks  ="1 month")+
  labs(title="E. Daily temperature maxima", y="Temperature (°C)", x="")+
  theme(text=element_text(size=14,  family="serif"))
...
daymean_min_plot <- 
  ...
  theme(axis.text.x = element_text(angle=0, margin = margin(t=5, r=50)))+
  scale_x_date(date_labels="%b",date_breaks  ="1 month")+
  labs(title="F. Daily temperature minima", y="Temperature (°C)", x="")+
  theme(text=element_text(size=14,  family="serif"))
...
OT_PA.plot <-
  ...
  scale_color_manual(values=c("orange2", "black")) + theme_bw()+
